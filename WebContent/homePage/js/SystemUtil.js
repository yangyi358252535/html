...
function DivWindowOpen(divWidth, divHeight, title, url, data) {
	...
	// 交换X图片
	$("#x").hover(function() {
		$(this).prop("src", "../images/Close-2.gif");
	}, function() {
		$(this).prop("src", "../images/Close-1.gif");
	});
	...
	$("#h3_head").click(
			function() {
				if ($("#topDIV").is(":visible")) {
					...
					$(this).find("a:eq(1)").prop('class',
							'hospital-icon ui-icon-newwin');
					$("#h3_detail").find("a:eq(1)").html("最小化");
					$("#h3_detail").find("a:eq(0)").prop('class',
	...
	$("#h3_detail").click(
			function() {
				if ($("#bottomDIV").is(":visible")) {
					...
					$("#h3_head").find("a:eq(1)").prop('class',
							'hospital-icon ui-icon-minusthick');
					$("#topDIV").slideDown(700);
				} else {
					$("#bottomDIV").slideDown(700);
...
function removeAttribuate(id) {
	...
	$("#" + id).prop('style', 'right: 30px; top: 13px;');
}
function showAttribuate(id) {
	$("#" + id).prop('style', 'right: 30px; top: 80px;');
}
...
function showAttribuate1(id) {
	$("#" + id).prop('style', 'right: 30px; top: 53px;');
}
/**
 * 采购模块的搜索栏的显示与隐藏
 */
